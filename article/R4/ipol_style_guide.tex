...
\ipolSetTitle{A MATLAB SMO Implementation to Train a SVM Classifier: Applied Multi-Style License Plate Numbers Recognition}

% Author names must be separated with commas (,), not "and" or "&".
\ipolSetAuthors{Pablo Negri \ipolAuthorMark{1},
                \ipolAuthorMark{2}}
...
\ipolSetAffiliations{%
\ipolAuthorMark{1} CONICET, Av. Rivadavia 1917, Buenos Aires, Argentina\\
\ipolAuthorMark{2} Instituto de Tecnologia, Universidad Argentina de la Empresa (UADE), Lima 717, Buenos Aires, Argentina
                   (\texttt{pnegri@uade.edu.ar})}
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=.8\linewidth]{/images/svm_margin.pdf}
...
\DontPrintSemicolon
\Input{$\mathbf{K}$, $C$, $\mathbf{\alpha}_0$}
\Output{$\mathbf{\alpha}$}
examineAll = 1
