...
global K E Targets Alphas C b GAMMA;
global D
...
for i=1:npts
    ...
    for j=1:npts
        r2 = r(:,j);
        K(i,j) = computeKernelFunction(r1/sum(r1),r2/sum(r2),KernelFunction);
    end
end
...
[CR_TH,CD_TH] = computeOCRCogniteThresholds(r,y,TabAlphas,TabB,KernelFunction);
%%%%%%%%%%%% TEST %%%%%%%%%%%
disp('Testing...');
o = zeros(size(ytest));
cr = zeros(size(ytest));
...
for i=1:length(ytest)
    ...
    for k=1:10
        ...
        cum_out = 0;
        for j=1:length(y)
            cum_out = cum_out + computeKernelFunction(r(:,j)/sum(r(:,j)),rt/sum(rt),KernelFunction) * TabAlphas(k,j) * Targets(j);
        end
...
save(fullfile('data','svmSMOmultipliers.mat'),'r','y','TabB','TabAlphas','CR_TH','CD_TH');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf('| Raw data - SVM classification   | %12s | \n', sprintf('%3.1f',mean(diag(MatConf)) ));
fprintf('--------------------------------------------------\n');
...
% | Method                          | Accuracy (%) |
% --------------------------------------------------
% | Raw data - Knn classification   |         74.9 | 
% | HOG data - Knn classification   |         96.1 | 
% | Raw data - SVM classification   |         90.0 | 
% --------------------------------------------------
% -------------------------------------------------------------------------------------
% Reliability Measures
% Percentage of samples with validated outputs: 100.00 
